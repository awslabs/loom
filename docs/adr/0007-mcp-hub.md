...
4. **Autorização = conta → agents invocáveis → McpServerAccess → tools.**
   Na Fase 1 a allowlist efetiva é a **união** das tools permitidas por
   todos os agents que o usuário pode invocar (mesma regra de ACL de
   agente do Chat). Deny-by-default se não houver regra.
...
| 3 | ACL user→tool nova, ignorando agents | Adiada: duplicaria modelo; Fase 1 reusa `McpServerAccess` via agents invocáveis. |
| 4 | Um Hub session = um agent fixo | Adiada como default; útil como opção depois. Default Fase 1 = **união**. |
...
3. [018 — Allowlist](../specs/018-mcp-hub-allowlist.md) (união por agents invocáveis)
