...
- **Platform Catalog** (default, Home) — Browse and manage agents, memory resources, MCP servers, and A2A agents. When the (opt-in) registry is enabled for governance and discovery, agents are auto-registered on deployment and must be approved before end-users can access them, and MCP servers/A2A agents must also be approved before use in agent deployments — approval status and actions (`RegistryStatusBadge`/`RegistryActions`) appear inline on each resource's card/detail view rather than as a separate aggregate Catalog section (the standalone Registry-records browsing section was removed as part of issue #37's visual redesign).
...
- Backend evaluations router (`routers/evaluations.py`, read-only, `agent:read`, `loom:group` enforced via `get_agent_or_404`): `GET /api/agents/{id}/evaluations` lists the AgentCore online evaluation configs and batch evaluation runs that evaluate the agent, with sampling, evaluators, last-evaluated time, and per-evaluator batch averages. A source belongs to the agent when its CloudWatch data source reads only the agent's runtime log groups (`/aws/bedrock-agentcore/runtimes/{runtime_id}-{qualifier}`), or when it names one of the agent's OTEL service names (`{runtime_name}.{qualifier}`); results of such shared sources are filtered to the agent's service names. `GET /api/agents/{id}/evaluations/results?source_type=online|batch&source_id=...&days=7` validates `source_id`, re-reads that one source from AWS, returns 404 unless it evaluates this agent, then reads its `gen_ai.evaluation.result` records (dedicated results log group, or the agent's own log group when `resultDestination` is `SOURCE_LOG_GROUP`), newest first, capped at 1,000 records, and returns per-trace scores (value, label, judge explanation). `GET /api/agents/{id}/evaluations/traces/{trace_id}/exchange` (hex trace IDs only) returns the evaluated prompt (last user text message) and final answer from the runtime's `otel-rt-logs` stream. Loom creates no evaluation resources and stores no scores.
- Frontend Evaluations tab (`components/AgentEvaluationsPanel.tsx`) on `AgentDetailPage`: one card per evaluation source (live-traffic config or test run) and a table of evaluated sessions / test cases with expandable prompt, answer and per-evaluator explanations.
...
### Phase 34 — Frontend Design System Refresh (issue #37) *(Complete)*
- **Two-theme system:** reduced the frontend's ten selectable themes (Ayu, Catppuccin, Dracula, Everforest, Nord, Rosé Pine, Solarized, Tokyo Night) down to one light and one dark theme, both a cool-neutral-gray palette with a blue accent. The grouped Light/Dark theme-picker dropdown was replaced by a single sun/moon toggle. See [`frontend/SPECIFICATIONS.md`](frontend/SPECIFICATIONS.md) § 11 "Theme System" for the token values.
- **Page-by-page visual modernization:** every remaining page (Catalog, Agent Detail/Invoke, Session/Invocation Detail, Integrations, Security, Analytics, Settings) was rebuilt around a small set of new shared components — `StatusPill` (dot + mono-caps status, replacing plain `Badge` for lifecycle/health everywhere), `CopyField`, `ExpandableRow`, `SettingsCard`/`SettingsRow`, `ViewModeToggle` — and a consistent card grammar (header · optional cost/metric hero · 2-col definition grid · footer with a "Details" link). See [`frontend/SPECIFICATIONS.md`](frontend/SPECIFICATIONS.md) § 11 "Page Modernization — Shared Components (issue #37)" for the full component/pattern inventory and the deliberate scope trims (no overflow-menu/`AlertDialog` primitives were introduced; MCP reachability approximates from existing status data; Models-tab family/generation bands and default-model chips were not built for lack of backing metadata).
- **Catalog Registry section removed:** the aggregate Registry-records browsing section (added in Phase 22, folded into Catalog in issue #20) was removed from the Platform Catalog page. Registry approval status/actions remain available inline on each resource's own card/detail view (`RegistryStatusBadge`/`RegistryActions`); the standalone `RegistryPage` component is currently unreferenced by any route.
- **Sidebar resource counts:** Memory and Integrations sidebar items now show a live count badge, matching the one Agents already had (Integrations shows the MCP + A2A sum). Counts are fetched independently in `App.tsx`, gated the same way as the existing `registryEnabled` fetch (`isAuthenticated` + the relevant `*:read` scope) rather than depending on whichever sub-page happens to be mounted — the Integrations tab counts specifically must be fetched by `IntegrationsPage` itself, since Radix `Tabs` unmounts inactive `TabsContent` and the not-yet-opened tab's own data fetch otherwise never runs.

### Phase 35 — OAuth2 Token-Endpoint Trust Check (SSRF/token-disclosure follow-up) *(Complete)*
- **Context:** a security report against commit `8c658d6` identified two backend issues. The first — the auth dependency granting a `t-admin`/`g-admins-super` identity to *every* request, including unauthenticated ones, whenever no Cognito/IdP is configured — was already fixed and released in 1.6.1 (fail-closed: requires an explicit `LOOM_ALLOW_UNAUTHENTICATED_LOCAL_DEV` opt-in plus a loopback client; see `backend/app/dependencies/auth.py` and `backend/tests/test_auth.py`/`test_scopes.py`). The second — SSRF via a user-supplied OAuth2 well-known URL, with the discovered `token_endpoint` receiving either the resource's own `client_secret` (M2M) or, in on-behalf-of mode, the calling *user's own real access token* — had its internal-address/metadata reach closed in the same 1.6.1 pass (PR #32, `net_guard.safe_get`/`safe_post`), but a residual gap remained: those guards still permit *any* public HTTPS host, so a malicious or compromised `mcp:write`/`a2a:write` admin could still point the exchange at a public server they control.
- **Fix — deployment-configured-IdP allowlist:** `net_guard.get_trusted_oauth_hosts()`/`is_trusted_oauth_host()` require the resolved `token_endpoint` to match a host already trusted as an identity provider (`IdentityProvider.issuer_url`, `AuthorizerConfig.discovery_url`) before `mcp.py`'s `_get_oauth2_token`/`_get_obo_token` or `a2a.py`'s `_get_oauth2_token` will send anything to it. Both trust-anchor tables are populated only through admin flows gated by a higher-trust scope than `mcp:write`/`a2a:write`, so the delegated-admin persona that can register a resource can no longer unilaterally redirect a token exchange to infrastructure of their own choosing — closing both the SSRF-to-arbitrary-public-host vector and, critically, the cross-user OBO token disclosure. See [`backend/SPECIFICATIONS.md`](backend/SPECIFICATIONS.md) § 7 "`services/net_guard.py`" for the full design and its one operational consequence (a brand-new downstream IdP must be registered as an Authorizer config or Identity Provider before OBO/M2M to it works).
- **Widened during review — every other OAuth2/OIDC outbound call, not just MCP/A2A:** an audit for the same bug class elsewhere in the backend found `services/oidc.py` (`fetch_discovery`), `services/jwt_validator.py` (`_get_jwks`), `services/token.py`, `services/authorizer_linking.py`, `services/cognito.py`, and `routers/auth.py`'s `/api/auth/token` all still using raw `urllib.request`/`httpx` calls with, at best, a URL-scheme check — none of them resolved-IP-validated, so all could still reach internal/metadata addresses. All six were moved onto `net_guard.safe_get`/`safe_post`; the two whose `token_endpoint` comes from a discovery document they resolve themselves (`token.py`, `authorizer_linking.py`) also gained the `is_trusted_oauth_host()` allowlist check, since a compromised discovery response is the same attack there as it is for MCP/A2A. `oidc.require_https_url` was removed as dead code once every caller switched to `safe_get`/`safe_post`. Deliberately left as-is: `services/litellm.py`/`model_catalog.py`'s LiteLLM proxy `base_url` calls — that setting is `settings:write`-gated (not a per-resource, lower-trust field) and documented to legitimately point at `http://localhost:<port>` in local dev, which `net_guard`'s guards unconditionally block. See [`backend/SPECIFICATIONS.md`](backend/SPECIFICATIONS.md) § 7 "`services/net_guard.py`" for the full list.
- **Scope boundary:** the fix covers every backend-initiated OAuth2/OIDC outbound call. The report also traced a related OBO forwarding path through `routers/invocations.py` into the deployed AgentCore runtime itself (`agents/strands_agent`), which executes outside this backend process and was not exercised end-to-end by the report; it remains a separate, not-yet-addressed surface.

### Phase 36 — Resource-Group Isolation on Single-Object Routes + IdP Group-Mapping Self-Escalation Guard *(Complete)*
- **Finding 1 (H1-3954919): broken `loom:group` isolation on ~30 single-object routes.** List routes for agents/memories correctly filter by the `loom:group` tag, but the shared single-object fetch-by-ID helpers — `routers/utils.py`'s `get_agent_or_404()` and the inline `Memory` lookups in `memories.py` — resolved by ID alone with no group check, so any authenticated user in one resource group could read/update/delete/export another group's agents and memories, including minting a live Cognito access token for any agent via `POST /api/agents/{id}/token`, purely by guessing/enumerating IDs. Confirmed as a real, currently-exploitable bug — not expected behavior.
  - **Fix:** a new shared `check_resource_group_access()` in `routers/utils.py` — the same logic the agent invoke route already applied correctly — is now called from `get_agent_or_404()` (closing all 21 call sites across `agents.py`/`credentials.py`/`integrations.py` in one place) and from a new `_get_memory_or_404()` in `memories.py` (6 call sites), plus directly in the `/api/agents/{id}/token` handler. See [`backend/SPECIFICATIONS.md`](backend/SPECIFICATIONS.md) § 6 "Tag-based resource isolation" for the full semantics and the explicitly out-of-scope follow-ups (`ManagedRole` — same shape, not currently exploitable; `AuthorizerConfig`/credentials/permission requests — globally scoped by design; MCP servers/A2A agents — no group concept exists yet at all).
  - **Owner / ECD:** heeki; fixed same-day as triage, this branch.
- **Finding 2: IdP group-mapping self-escalation to `g-admins-super`.** `create_identity_provider`/`update_identity_provider` in `routers/identity_providers.py` are gated only by `security:write`, and persisted the caller's `group_mappings` table verbatim; the login path's `_map_external_groups()` then maps external IdP group claims through that table with no floor against reserved/privileged groups. A `g-admins-security` admin (only `security:read/write`, `settings:read/write`, `tagging:read`) could map an external group they control to `g-admins-super` and, by authenticating through that IdP, obtain the full super-admin scope set including `admin:write`. Confirmed as a real, currently-exploitable bug via the reporter's own local PoC against the shipped `_map_external_groups`/`derive_scopes` code — not expected behavior.
  - **Fix:** `_assert_group_mappings_within_caller_scopes()` rejects (403) any `group_mappings` write where the union of scopes granted by the named target groups exceeds the caller's own scopes — a general "no delegation beyond what you hold" rule rather than a hardcoded denylist, so it also blocks escalating to any other admin group (e.g. `g-admins-mcp`) the caller doesn't already hold. See [`backend/SPECIFICATIONS.md`](backend/SPECIFICATIONS.md) § 12 "Group Claim Mapping" for the full design.
  - **Owner / ECD:** heeki; fixed same-day as triage, this branch.
