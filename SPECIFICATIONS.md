...
- **Scope boundary:** the fix covers the backend's own OAuth2 exchange code paths (`test-connection`, `tools/refresh` for MCP; `test-connection`, Agent Card refresh for A2A). The report also traced a related OBO forwarding path through `routers/invocations.py` into the deployed AgentCore runtime itself (`agents/strands_agent`), which executes outside this backend process and was not exercised end-to-end by the report; it remains a separate, not-yet-addressed surface.
